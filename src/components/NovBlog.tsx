...
const NovBlog: React.FC = () => {
  const [comments, setComments] = useState<Comment[]>(() => {
    // Initialize comments from session storage or empty array
    const storedComments = sessionStorage.getItem('blogComments');
  ...
  const handleCommentSubmit = () => {
    if (newComment.text.trim() && newComment.firstName.trim() && newComment.dogName.trim()) {
      ...
      // Store updated comments in session storage
      sessionStorage.setItem('blogComments', JSON.stringify(updatedComments));
      ...
      <Box sx={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <Box sx={{ mt: 6 }}>
          ...
          <Typography variant="h6" component="h3" sx={{ mt: 4, mb: 2, fontWeight: 'semibold' }}>
            Ready to give your dog the pampering they deserve this season?
          </Typography>
          <Typography variant="body1">
            Contact us to book a grooming session or visit our website for more information. We can't wait to make your pup look fabulous this November!
          </Typography>
        </Box>
      </Box>
    </Container>
